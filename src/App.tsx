...
function LogsPage({ state, onClear, notify }: { state: AppState; onClear: () => void; notify: NoticeHandler }) {
  ...
  return <>
    <SectionHeader eyebrow="反代服务 / 可观测性" title="请求日志" description="查看今天的请求、账号调度、响应耗时和错误分类（每日零点自动清零）。" action={<div className="header-actions"><button className="button ghost compact" onClick={exportLogs}><Upload size={15} />导出当前筛选 JSON</button></div>} />
